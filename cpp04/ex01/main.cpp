...
int main()
{
	...
	Dog* first = dynamic_cast<Dog*>(animals[0]);
	first->getBrain().setIdea(0, "Bone...");
	first->getBrain().setIdea(1, "Woof...");
	std::cout << first->getBrain().getIdea(0) << std::endl;
	std::cout << first->getBrain().getIdea(1) << std::endl;
	...
	if (dog)
	{
		...
		dog->getBrain().setIdea(1, "Yap Yap!");
	...
	if (cat)
	{
		cat->getBrain().setIdea(0, "Milk...");
		cat->getBrain().setIdea(1, "Meoww");
